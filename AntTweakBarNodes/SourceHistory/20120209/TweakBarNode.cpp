...
// Destructor
// リソース、特にアンマネージ度リソースを積極的に解放するためのメソッド
...
// Finalizer
// リソースの解放し忘れによる被害を最小限に抑えるためのメソッド
...
void TweakBarNode::Evaluate(int SpreadMax)
{
	...
	//if(isConnected && (bool)FEnabled[0])
	if(isConnected)
	{
		//--------------------------------------Events
		
		//本来はイベント関連は複数のTweakBarに対して1回でいい
		//SendEventsNodeはそのための実験
...
DeviceData^ TweakBarNode::CreateDeviceData(Device^ device)
{
	...
	//capture StateBlock of CreateDeviceData called.
	//block->Capture();	//capture here will NOT work with open patch with connected to Renderer
...
void TweakBarNode::Render(Device^ device, DeviceData^ deviceData)
{
	...
	//capture stateblock BEFORE UpdateDeviceData call
	if(GBarCount == 0) block->Capture();
...
void TweakBarNode::InitTweakBar(Device^ device)
{
	...
	if(GBarCount == 0)
	{
		...
		//---------------------------------------------------------画面サイズをセット
		//いきなりInput pinの値を参照すると、Rendererにつなげたまま
		//Patchを開いたときに値が0になってしまうのでダミー値を使う。
		//UpdateDeviceData()でもう一度セットする
