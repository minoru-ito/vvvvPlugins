...
TweakBarNode::TweakBarNode(IPluginHost2^ host) : DXLayerOutPluginBase(host, false, false)
{
	...
	FArrayColor = gcnew array<float, 2>(MAX_COUNT, 4);			//4つのfloatを持つ配列の配列（RGBA）
	FArrayDirection = gcnew array<double, 2>(MAX_COUNT, 3);		//3つのdoubleを持つ配列の配列（XYZ）
	FArrayQuaternion = gcnew array<double, 2>(MAX_COUNT, 4);	//4つのdoubleを持つ配列の配列（XYZW）
	*/
...
void TweakBarNode::InitTweakBar(Device^ device)
{
	...
	*/
	...
	//TweakBarの作成（複数のTweakBarを作成したときに名前がかぶらないようにしている）
	bar = TwNewBar(name);
